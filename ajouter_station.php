...
<div id="page-wrapper">
    <div class="container-fluid">
        <div class="max-w-9xl mx-auto px-4 py-1">
            <!-- Page Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900">Ajouter Agence</h1>
                <p class="mt-2 text-sm text-gray-600">Créer une nouvelle agence dans le système</p>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8">
                <form class="space-y-8" enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == 'POST') {
                        ...
                        $qte = $_POST["qte"];
                        $ind = $_POST["ind"];
                        $sth = $db->prepare('INSERT INTO station VALUES(NULL,:lib, :qte, :ind)');
                        $sth->bindParam(':lib', $lib);
                        $sth->bindParam(':qte', $qte);
                        $sth->bindParam(':ind', $ind);
                        $sth->execute();

                        setFlashMessage("Succées d'ajout de agence");
