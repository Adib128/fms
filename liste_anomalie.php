    ...
    <div class="mx-auto flex flex-col gap-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">Gestion des anomalies liées aux systèmes.</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="<?= url('ajouter-anomalie') ?>" class="btn-primary">
        ...
        <div class="panel">
            ...
            <div class="panel-body overflow-x-auto">
                ...
                <div id="tableContainer" style="display: none;">
                    ...
                    <table class="table compact-table" id="anomalieTable">
                        <tfoot>
                            <tr>
                                <th>Désignation</th>
                                <th>Système</th>
                        ...
                        <tbody>
                            ...
                            try {
                                $stmt = $db->query("
                                    SELECT an.*, s.designation as systeme_nom, a.nom as atelier_nom 
                                    FROM anomalie an 
                                    JOIN systeme s ON an.id_system = s.id 
                                    JOIN atelier a ON s.id_atelier = a.id
                                    ORDER BY an.designation ASC
                                ");
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    ?>
                                    <tr>
                                        <td class="font-medium"><?= htmlspecialchars($row['designation']) ?></td>
                                        <td><?= htmlspecialchars($row['systeme_nom']) ?></td>
                                        <td><?= htmlspecialchars($row['atelier_nom']) ?></td>
                                        <td class="text-right">
                                            <div class="flex gap-1 justify-center">
                                                <a href="<?= url('modifier-anomalie') ?>?id=<?= $row['id'] ?>" 
                                                ...
                                                <a href="<?= url('supprimer-anomalie') ?>?id=<?= $row['id'] ?>" 
                                                   onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette anomalie ?');" 
                                ...
                                echo '<tr><td colspan="4" class="text-center text-red-600">Erreur: ' . htmlspecialchars($e->getMessage()) . '</td></tr>';
                            }
                            ?>
                        </tbody>
                    </table>
...
$(document).ready(function () {
    var titreRapport = "Anomalies";

    const table = $('#anomalieTable').DataTable({
        bSort: false,
        deferRender: true,
        autoWidth: false,
        scrollX: false,
        ...
        initComplete: function (settings, json) {
            setTimeout(function() {
                try {
                    ...
                    $('#anomalieTable tfoot th').each(function (i) {
                        const $this = $(this);
                        if ($this.length) {
