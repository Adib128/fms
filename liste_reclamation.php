...
<div id="page-wrapper">
    <div class="mx-auto flex flex-col gap-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="page-title">Réclamations d'anomalie</h1>
                <p class="text-sm text-slate-500">Suivi des réclamations et anomalies signalées.</p>
            </div>
            <?php if ($userProfile !== 'responsable'): ?>
            <div class="flex items-center gap-3">
                <a href="<?= url('ajouter-reclamation') ?>" class="btn-primary">
        ...
        <div class="panel">
            ...
            <div class="panel-body overflow-x-auto">
                ...
                <div id="tableContainer" style="display: none;">
                    ...
                    <table class="table compact-table" id="reclamationsTable">
                        <tfoot>
                            <tr>
                                <th>Date</th>
                                <th>Véhicule</th>
                        ...
                        <tbody>
                            ...
                            try {
                                $stmt = $db->query("
                                    ...
                                    FROM reclamation r
                                    LEFT JOIN bus b ON r.id_vehicule = b.id_bus
                                    LEFT JOIN chauffeur c ON r.id_chauffeur = c.id_chauffeur
                                    LEFT JOIN station s ON r.id_station = s.id_station
                                    ORDER BY r.date DESC, r.id DESC
                                ");
                                
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    $etatClass = $row['etat'] === 'traitée' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-orange-100 text-orange-800 border-orange-200';
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-medium <?= $etatClass ?>">
                                        ...
                                        <td class="text-right">
                                            <div class="flex gap-1 justify-center">
                                                <?php if ($userProfile !== 'responsable'): ?>
                                                <a href="<?= url('modifier-reclamation') ?>?id=<?= $row['id'] ?>" 
                                                ...
                                                <a href="<?= url('supprimer-reclamation') ?>?id=<?= $row['id'] ?>" 
...
$(document).ready(function () {
    var titreRapport = "Réclamations d'anomalie";

    const table = $('#reclamationsTable').DataTable({
        bSort: false,
        deferRender: true,
        autoWidth: false,
        scrollX: false,
        ...
        initComplete: function (settings, json) {
            setTimeout(function() {
                try {
                    ...
                    $('#reclamationsTable tfoot th').each(function (i) {
                        const $this = $(this);
                        if ($this.length) {
