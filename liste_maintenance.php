    ...
    <div class="mx-auto flex flex-col gap-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">Gestion des techniciens de maintenance.</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="<?= url('ajouter-maintenance') ?>" class="btn-primary">
        ...
        <div class="panel">
            ...
            <div class="panel-body overflow-x-auto">
                ...
                <div id="tableContainer" style="display: none;">
                    ...
                    <table class="table compact-table" id="maintenanceTable">
                        <tfoot>
                            <tr>
                                <th>Matricule</th>
                                <th>Nom</th>
                        ...
                        <thead>
                            <tr>
                                ...
                                <th>Nom</th>
                                <th>Spécialité</th>
                                <th class="search_select">Atelier</th>
                        ...
                        <tbody>
                            ...
                            try {
                                $stmt = $db->query("
                                    SELECT m.*, a.nom as atelier_nom
                                    FROM maintenance m
                                    LEFT JOIN atelier a ON m.id_atelier = a.id
                                    ORDER BY m.nom ASC
                                ");
                                ...
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    ?>
                                    <tr>
                                        <td class="font-medium"><?= htmlspecialchars($row['matricule'] ?? '-') ?></td>
                                        <td class="font-medium"><?= htmlspecialchars($row['nom']) ?></td>
                                        <td><?= htmlspecialchars($row['specialite'] ?? '-') ?></td>
                                        <td><?= htmlspecialchars($row['atelier_nom']) ?></td>
                                        <td class="text-right">
                                            <div class="flex gap-1 justify-center">
                                                <a href="<?= url('modifier-maintenance') ?>?id=<?= $row['id'] ?>" 
                                                ...
                                                <a href="<?= url('supprimer-maintenance') ?>?id=<?= $row['id'] ?>" 
                                                   onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce technicien ?');" 
...
$(document).ready(function () {
    var titreRapport = "Liste des techniciens";
        ...
        initComplete: function (settings, json) {
            setTimeout(function() {
                try {
                    ...
                    $('#maintenanceTable tfoot th').each(function (i) {
                        const $this = $(this);
                        if ($this.length) {
