...
if ($id === 0) {
    $_SESSION['message'] = "ID de technicien invalide.";
...
try {
    ...
    if (!$technicien) {
        $_SESSION['message'] = "Technicien introuvable.";
        header('Location: ' . url('liste-maintenance'));
        exit;
    }
} catch (PDOException $e) {
    $_SESSION['message'] = "Erreur lors de la récupération du technicien.";
    header('Location: ' . url('liste-maintenance'));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $db->prepare('UPDATE maintenance SET matricule = ?, nom = ?, specialite = ?, id_atelier = ? WHERE id = ?');
            $stmt->execute([$matricule, $nom, $specialite, $id_atelier, $id]);
            
            $_SESSION['message'] = "Technicien modifié avec succès.";
            header('Location: ' . url('liste-maintenance'));
            exit;
        } catch (PDOException $e) {
            $errors[] = "Erreur lors de la modification du technicien : " . $e->getMessage();
        }
    }
}
...
$formData = $_SERVER['REQUEST_METHOD'] === 'POST' ? $_POST : $technicien;
?>
...
<div id="page-wrapper">
    <div class="mx-auto flex flex-col gap-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="page-title">Modifier le technicien</h1>
            </div>
        </div>
                        ...
                        <div class="md:col-span-8">
                            <select id="id_atelier" name="id_atelier" required class="tom-select w-full">
                                ...
                                <?php foreach ($ateliers as $atelier): ?>
                                    <option value="<?= $atelier['id'] ?>" <?= ($formData['id_atelier'] == $atelier['id']) ? 'selected' : '' ?>>
