...
try {
    ...
    // Système Statistics
    $stmtSystemes = $db->prepare("
        SELECT 
            s.id,
            s.designation,
            a.nom as atelier_nom,
            COUNT(DISTINCT o.id) as total_ordres,
            COUNT(DISTINCT oa.id_anomalie) as total_anomalies
        FROM systeme s
        LEFT JOIN atelier a ON s.id_atelier = a.id
        LEFT JOIN ordre o ON s.id = o.id_system AND o.date BETWEEN :start AND :end
        LEFT JOIN ordre_anomalie oa ON o.id = oa.id_ordre
        LEFT JOIN anomalie an ON oa.id_anomalie = an.id AND an.id_system = s.id
        GROUP BY s.id, s.designation, a.nom
        ORDER BY total_ordres DESC
    ");
    $stmtSystemes->execute($params);
    $systemes = $stmtSystemes->fetchAll(PDO::FETCH_ASSOC);
    ...
    $stmtTechniciens = $db->prepare("
        SELECT 
            m.id,
            m.nom,
            m.matricule,
            COUNT(DISTINCT CASE WHEN oit.type = 'prévu' THEN oit.id_ordre_intervention END) as interventions_prevues,
            COUNT(DISTINCT CASE WHEN oit.type = 'réalisé' THEN oit.id_ordre_intervention END) as interventions_realisees,
            COUNT(DISTINCT CASE WHEN oit.type = 'prévu' THEN oi.id_ordre END) as ordres_prevus,
            COUNT(DISTINCT CASE WHEN oit.type = 'réalisé' THEN oi.id_ordre END) as ordres_realises
        FROM maintenance m
        LEFT JOIN ordre_intervention_technicien oit ON m.id = oit.id_technicien
        LEFT JOIN ordre_intervention oi ON oit.id_ordre_intervention = oi.id
        LEFT JOIN ordre o ON oi.id_ordre = o.id AND o.date BETWEEN :start AND :end
        GROUP BY m.id, m.nom, m.matricule
        HAVING interventions_prevues > 0 OR interventions_realisees > 0
        ORDER BY interventions_realisees DESC, interventions_prevues DESC
    ");
    $stmtTechniciens->execute($params);
    ...
    <div class="mx-auto flex flex-col gap-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 no-print">
            <div class="flex items-center justify-between w-full lg:w-auto">
                <div>
                    <h1 class="page-title">Statistiques de Maintenance</h1>
                    <p class="text-sm text-gray-600 mt-1">Analyse détaillée des ateliers, systèmes et anomalies</p>
                </div>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4 flex-1 lg:max-w-3xl">
                <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 w-full">
                    <form method="GET" class="flex flex-col sm:flex-row items-end gap-3">
                        <div class="flex-1 w-full">
                            <label for="start_date" class="block text-xs font-semibold text-gray-500 uppercase mb-1">Date Début</label>
                            <input type="date" name="start_date" id="start_date" value="<?= htmlspecialchars($startDate) ?>" class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                        </div>
                        <div class="flex-1 w-full">
                            <label for="end_date" class="block text-xs font-semibold text-gray-500 uppercase mb-1">Date Fin</label>
                            <input type="date" name="end_date" id="end_date" value="<?= htmlspecialchars($endDate) ?>" class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                        </div>
                        <div class="flex gap-2">
                            <button type="submit" class="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm" title="Filtrer">
                                <span class="iconify h-5 w-5" data-icon="mdi:filter"></span>
                            </button>
                            <a href="<?= url('statistique-maintenance') ?>" class="p-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 transition-colors" title="Réinitialiser">
                                <span class="iconify h-5 w-5" data-icon="mdi:refresh"></span>
                            </a>
                        </div>
                    </form>
                </div>
            ...
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-blue-100 text-xs mt-1">Sur la période</p>
                    </div>
            ...
            <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-green-100 text-xs mt-1">Sur la période</p>
                    </div>
            ...
            <div class="panel-body">
                ...
                <div class="overflow-x-auto">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200" id="atelierTableBody">
                            <?php foreach ($ateliers as $atelier): ?>
                                <tr class="hover:bg-gray-50 atelier-row" data-search="<?= strtolower(htmlspecialchars($atelier['nom'])) ?>">
                ...
                <div id="noAtelierResults" class="hidden text-center py-8 text-gray-500">
        ...
        <div class="panel">
            <div class="panel-heading flex items-center justify-between">
                <span>Analyse par Système</span>
                <span class="text-xs text-gray-500 font-normal"><?= count($systemes) ?> systèmes</span>
            </div>
            <div class="panel-body">
                <!-- Search Input -->
                <div class="mb-4">
                    <input 
                        type="text" 
                        id="systemeSearch" 
                        placeholder="Rechercher un système ou atelier..." 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Système</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Atelier</th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Total Ordres</th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Anomalies</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200" id="systemeTableBody">
                            <?php foreach ($systemes as $systeme): ?>
                                <tr class="hover:bg-gray-50 systeme-row" data-search="<?= strtolower(htmlspecialchars($systeme['designation'] . ' ' . ($systeme['atelier_nom'] ?? ''))) ?>">
                                    <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">
                                        <?= htmlspecialchars($systeme['designation']) ?>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">
                                        <?= htmlspecialchars($systeme['atelier_nom'] ?? 'N/A') ?>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-center text-gray-900 font-semibold">
                                        <?= number_format($systeme['total_ordres']) ?>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-center">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
                                            <?= number_format($systeme['total_anomalies']) ?>
                                        </span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                ...
                <div id="noSystemeResults" class="hidden text-center py-8 text-gray-500">
        ...
        <div class="panel">
            <div class="panel-heading flex items-center justify-between">
                <span>Performance par Technicien</span>
                <span class="text-xs text-gray-500 font-normal"><?= count($techniciens) ?> techniciens</span>
            </div>
            <div class="panel-body">
                <!-- Search Input -->
                <div class="mb-4">
                    <input 
                        type="text" 
                        id="technicienSearch" 
                        placeholder="Rechercher un technicien (matricule ou nom)..." 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Interventions Prévues</th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Interventions Réalisées</th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Taux Réalisation</th>
                            </tr>
                        </thead>

                        <tbody class="bg-white divide-y divide-gray-200" id="technicienTableBody">
                            ...
                            <?php else: ?>
                                <?php foreach ($techniciens as $tech): ?>
                                    <tr class="hover:bg-gray-50 technicien-row" data-search="<?= strtolower(htmlspecialchars(($tech['matricule'] ?? '') . ' ' . $tech['nom'])) ?>">

                                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">
                                            <?= htmlspecialchars($tech['matricule'] ?? 'N/A') ?>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                            <?= htmlspecialchars($tech['nom']) ?>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-center text-gray-600">
                                            <?= number_format($tech['interventions_prevues']) ?>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-center font-semibold text-gray-900">
                                            <?= number_format($tech['interventions_realisees']) ?>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-center">
                                            <?php 
                                            $tauxRealisation = $tech['interventions_prevues'] > 0 
                                                ? round(($tech['interventions_realisees'] / $tech['interventions_prevues']) * 100, 1) 
                                                : 0;
                                            $colorClass = $tauxRealisation >= 75 ? 'text-green-600' : ($tauxRealisation >= 50 ? 'text-amber-600' : 'text-red-600');
                                            ?>
                                            <span class="font-semibold <?= $colorClass ?>"><?= $tauxRealisation ?>%</span>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                ...
                <div id="noTechnicienResults" class="hidden text-center py-8 text-gray-500">
...
// Atelier Search Functionality
document.getElementById('atelierSearch').addEventListener('input', function(e) {
    const searchTerm = e.target.value.toLowerCase();
    const rows = document.querySelectorAll('.atelier-row');
    const noResults = document.getElementById('noAtelierResults');
...
// Système Search Functionality
document.getElementById('systemeSearch').addEventListener('input', function(e) {
    const searchTerm = e.target.value.toLowerCase();
    const rows = document.querySelectorAll('.systeme-row');
    const noResults = document.getElementById('noSystemeResults');
    const tableBody = document.getElementById('systemeTableBody');
    let visibleCount = 0;
...
// Technicien Search Functionality
document.getElementById('technicienSearch').addEventListener('input', function(e) {
    const searchTerm = e.target.value.toLowerCase();
    const rows = document.querySelectorAll('.technicien-row');
    const noResults = document.getElementById('noTechnicienResults');
...
function downloadPDF() {
    ...
    const opt = {
        margin: [10, 10],
        filename: 'statistiques_maintenance_' + new Date().toISOString().slice(0,10) + '.pdf',
