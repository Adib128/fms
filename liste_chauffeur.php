...
require 'header.php'; ?>
<?php
$chauffeursStmt = $db->query('SELECT * FROM chauffeur ORDER BY id_chauffeur DESC');
$chauffeurs = $chauffeursStmt ? $chauffeursStmt->fetchAll(PDO::FETCH_ASSOC) : [];

$stationStmt = $db->query('SELECT id_station, lib FROM station');
$stations = $stationStmt ? $stationStmt->fetchAll(PDO::FETCH_KEY_PAIR) : [];
?>
    ...
    <div class="mx-auto flex max-w-9xl flex-col gap-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">Gérez les chauffeurs et leurs stations d'affectation.</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
                <a href="<?= url('ajouter-chauffeur') ?>" class="btn-primary">
        ...
        <div class="panel min-h-[800px]">
            <div class="panel-heading">
                <span>Chauffeurs enregistrés</span>
                <span class="badge">
                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M3 3v18h18" />
            ...
            <div class="panel-body overflow-x-auto">
                <table class="table" id="tab">
                    ...
                    <tbody>
                        <?php foreach ($chauffeurs as $row) : ?>
                            <tr>
                                <td class="font-medium text-slate-900"><?= htmlspecialchars($row['matricule'], ENT_QUOTES, 'UTF-8'); ?></td>
                                <td><?= htmlspecialchars($row['nom_prenom'], ENT_QUOTES, 'UTF-8'); ?></td>
                                <td>
                                    <?= htmlspecialchars($stations[$row['id_station']] ?? '-', ENT_QUOTES, 'UTF-8'); ?>
                                </td>
                                <td class="text-right">
                                    <div class="inline-flex items-center gap-2">
                                        <a
                                            href="<?= url('modifier-chauffeur') ?>?id=<?= (int) $row['id_chauffeur']; ?>"
                                        ...
                                        <a
                                            href="supprimer_chauffeur.php?id=<?= (int) $row['id_chauffeur']; ?>"
                                            onclick="return confirm('Voulez vous vraiment supprimer le chauffeur ?');"
...
<script>
    $(function () {
        const titreRapport = 'Liste des chauffeurs';
