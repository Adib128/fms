...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($code) || empty($parti) || empty($label)) {
        ...
        try {
            $stmt = $db->prepare('UPDATE checklist_items SET code = ?, label = ?, parti = ? WHERE id = ?');
            $stmt->execute([$code, $label, $parti, $id]);
            
            $_SESSION['message'] = 'Item de checklist modifié avec succès';
            
            header('Location: /liste-checklist-items');
            exit;
        } catch (PDOException $e) {
            $error = 'Une erreur est survenue lors de la modification de l\'item: ' . $e->getMessage();
        }
    }
}
...
try {
    ...
    $error = 'Une erreur est survenue lors du chargement de l\'item';
}
?>
        ...
        <div class="max-w-9xl mx-auto px-4 py-1">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Modifier un Item de Checklist</h1>
                        <p class="mt-2 text-sm text-gray-600">Mettre à jour les informations de l'item de checklist</p>
                    </div>
                ...
                <form method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               name="label" 
                               required 
                               value="<?= htmlspecialchars($item['label'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div>
                        ...
                        <select id="parti" 
                                ...
                                required 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="MOTEUR" <?= ($item['parti'] ?? '') === 'MOTEUR' ? 'selected' : '' ?>>MOTEUR</option>
                            <option value="TRANSMISSION" <?= ($item['parti'] ?? '') === 'TRANSMISSION' ? 'selected' : '' ?>>TRANSMISSION</option>
                            <option value="FREINAGE" <?= ($item['parti'] ?? '') === 'FREINAGE' ? 'selected' : '' ?>>FREINAGE</option>
                            <option value="SUSPENSION ET DIRECTION" <?= ($item['parti'] ?? '') === 'SUSPENSION ET DIRECTION' ? 'selected' : '' ?>>SUSPENSION ET DIRECTION</option>
                            <option value="ELECTRICITE AUTO" <?= ($item['parti'] ?? '') === 'ELECTRICITE AUTO' ? 'selected' : '' ?>>ELECTRICITE AUTO</option>
                            <option value="CARROSSERIE" <?= ($item['parti'] ?? '') === 'CARROSSERIE' ? 'selected' : '' ?>>CARROSSERIE</option>
                        </select>
                    </div>
