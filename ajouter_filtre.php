...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $db->prepare('INSERT INTO filter_types (name, usageFilter) VALUES (:name, :usageFilter)');
            $stmt->execute([
                ':name' => $name,
                ':usageFilter' => $usageFilter
            ]);
            
            $success = "Type de filtre ajouté avec succès !";
            
            // Clear form
            $_POST = [];
        } catch (PDOException $e) {
            $errors[] = "Erreur lors de l'ajout du type de filtre : " . $e->getMessage();
        }
    }
}
?>
...
<div id="page-wrapper">
    <div class="container-fluid">
        <div class="max-w-9xl mx-auto px-4 py-1">
            <!-- Page Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900">Ajouter Type de Filtre</h1>
                <p class="mt-2 text-sm text-gray-600">Créer un nouveau type de filtre pour les opérations d'entretien</p>
            </div>
                            ...
                            <select id="usageFilter" name="usageFilter" 
                                ...
                                <?php foreach ($compartiments as $compartiment): ?>
                                    <option value="<?= htmlspecialchars($compartiment, ENT_QUOTES, 'UTF-8'); ?>" 
                                            <?= (($_POST['usageFilter'] ?? '') === $compartiment ? 'selected' : ''); ?>>
