...
// Fetch existing atelier
$atelier = null;
try {
    $stmt = $db->prepare('SELECT * FROM atelier WHERE id = ?');
    $stmt->execute([$id]);
    $atelier = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$atelier) {
        $_SESSION['message'] = "Atelier introuvable.";
        header('Location: ' . url('liste-atelier'));
        exit;
    }
} catch (PDOException $e) {
    $_SESSION['message'] = "Erreur lors de la récupération de l'atelier.";
    header('Location: ' . url('liste-atelier'));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $db->prepare('UPDATE atelier SET nom = ? WHERE id = ?');
            $stmt->execute([$nom, $id]);
            
            $_SESSION['message'] = "Atelier modifié avec succès.";
            header('Location: ' . url('liste-atelier'));
            exit;
        } catch (PDOException $e) {
            $errors[] = "Erreur lors de la modification de l'atelier : " . $e->getMessage();
        }
    }
}
...
$formData = $_SERVER['REQUEST_METHOD'] === 'POST' ? $_POST : $atelier;
?>
...
<div id="page-wrapper">
    <div class="mx-auto flex flex-col gap-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="page-title">Modifier l'atelier</h1>
            </div>
        </div>
